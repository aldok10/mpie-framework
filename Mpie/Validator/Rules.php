...
namespace Mpie\Validator;

use Mpie\Validator\Exception\ValidateException;
...
class Rules
{
    ...
    public function max($key, $value, $max): bool
    {
        ...
        if (mb_strlen((string) $value, 'utf8') > (int) $max) {
            return $this->fail($this->validator->getMessage($key . '.' . __FUNCTION__, $key . 'maximum length' . $max));
        }
        return true;
    }
    ...
    public function min($key, $value, $min): bool
    {
        ...
        if (mb_strlen((string) $value, 'utf8') < (int) $min) {
            return $this->fail($this->validator->getMessage($key . '.' . __FUNCTION__, $key . 'minimum length' . $min));
        }
